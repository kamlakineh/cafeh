...
import { 
  ChefHat, Flame, Bell, Clock, Check, AlertTriangle, 
  Settings, Volume2, VolumeX, Database, HelpCircle, Sparkles, 
  MessageSquare, Plus, Trash2, Maximize2, Minimize2, ChevronRight, 
  ChevronLeft, Menu, X, TrendingUp, BarChart2, Printer, Play, 
  Pause, RotateCcw, User, CheckSquare, Shield, Clock3, AlertOctagon, 
  BookOpen, Compass, BadgeCheck
} from "lucide-react";
...
export default function KitchenDashboard({
  ...
  const [activeTab, setActiveTab] = React.useState<
    "Dashboard" | "New" | "Ready" | "Completed" | "Delayed" | "Timers" | "Inventory" | "Reports"
  >("Dashboard");
  ...
  const [selectedStation, setSelectedStation] = React.useState("Grill Station #2");
  const [activeChefsCount, setActiveChefsCount] = React.useState(4);
  const [emergencyAlert, setEmergencyAlert] = React.useState(false);
  const [viewDetailsOrderId, setViewDetailsOrderId] = React.useState<string | null>(null);
  ...
  const [currentChef, setCurrentChef] = React.useState({
    ...
    avatar: authenticatedStaff?.role === "Executive Chef" ? "https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=150" : "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150"
  });
  ...
  const [rejoiningOrderId, setRejoiningOrderId] = React.useState<string | null>(null);
  ...
  const [checklistState, setChecklistState] = React.useState<Record<string, Record<string, boolean>>>({});
  ...
  const [timers, setTimers] = React.useState<Array<{
    ...
    { id: "T1", label: "Grill #2 - Wagyu Patties", time: 240, maxTime: 240, running: true, type: "Grill" },
    { id: "T2", label: "Fryer #1 - Truffle Fries", time: 180, maxTime: 180, running: true, type: "Fryer" },
    { id: "T3", label: "Oven #1 - Brioche Buns", time: 60, maxTime: 60, running: false, type: "Oven" }
  ]);
  const [newTimerLabel, setNewTimerLabel] = React.useState("");
  const [newTimerType, setNewTimerType] = React.useState<"Grill" | "Fryer" | "Oven" | "Custom">("Custom");
  const [newTimerSeconds, setNewTimerSeconds] = React.useState("120");

  // Audio Alerts Synthesizer Engine (Standard Web Audio API)
  const playSound = React.useCallback((type: "ding-dong" | "beep" | "success" | "alarm") => {
  ...
  const [aiChatMessages, setAiChatMessages] = React.useState<Array<{ role: "user" | "ai"; text: string }>>([
    { role: "ai", text: "Salutations! I am the Gourmet Kitchen AI. I analyze live tickets, stock, and station telemetry to streamline preparation. Ask me how to optimize layout, predict prep times, or handle current backlogs." }
  ]);
  ...
  React.useEffect(() => {
    const checkUrgentOrdersInterval = setInterval(() => {
      const pendingAndPrep = orders.filter(o => ["Pending", "Preparing", "Cooking"].includes(o.status));
      const now = new Date();
  ...
  const updateStatusWithAudio = (id: string, nextStatus: Order['status'], extra?: Partial<Order>) => {
    playSound("success");
  ...
  const verifyPinAndReject = () => {
    if (enteredPin === "1234" || enteredPin === "7777" || enteredPin === "0000") {
      if (rejoiningOrderId) {
  ...
  const handleSendAiMessage = async () => {
    ...
    try {
      ...
      setAiChatMessages(prev => [...prev, { role: "ai", text: data.text || "Suggestion: Group burger tickets to grill #2 to save energy. Assign Elena to assembling." }]);
    } catch (err) {
      let answer = "Suggestion: Organize ticket groups by target temperatures. For Wagyu, maintain 550°F on Cast Iron. If bun backlog is building, reassign Station #3 to assist.";
      if (userMsg.toLowerCase().includes("prep") || userMsg.toLowerCase().includes("time")) {
        answer = "Predicted queue duration is currently 14 minutes. Suggest prioritizing VIP ticket ORD-101 immediately, as it contains Medium Rare Wagyu which has high sensitivity.";
      } else if (userMsg.toLowerCase().includes("stock") || userMsg.toLowerCase().includes("alert")) {
        answer = "Inventory check: Beef Patties are currently low (12 left). This will expire in 4 orders. Recommend clicking 'Request Restock' in Ingredient Alerts panel.";
      }
  ...
  const getOrdersForTab = () => {
    ...
    switch (activeTab) {
      ...
      case "Cooking":
        return orders.filter(o => o.status === "Cooking" || o.status === "Preparing");
      case "Ready":
      ...
      case "Delayed":
        return orders.filter(o => {
          ...
          const orderTime = new Date(o.createdAt);
          const diffMins = (now.getTime() - orderTime.getTime()) / 60000;
          return o.status === "Delayed" || diffMins > 10 || (o.notes && o.notes.toUpperCase().includes("DELAYED"));
        });
      default:
        return orders;
    }
  ...
  return (
    <div className="w-full bg-[#1A0F08] min-h-screen text-white font-sans flex flex-col relative" id="kitchen-display-system">
      ...
      {emergencyAlert && (
        <div className="bg-white text-[#1A0F08] animate-pulse py-3 px-4 flex justify-between items-center text-xs font-mono font-black uppercase tracking-wider z-[100] border-b-4 border-[#3D2516]">
          <div className="flex items-center space-x-3">
            ...
            <span>CRITICAL KITCHEN EMERGENCY ACTIVATED: CHECK STATION TEMPERATURES / POWER SAFETY</span>
          </div>
          <button 
        ...
        >
            ...
            <div className="flex items-center justify-between">
              {!sidebarCollapsed ? (
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <h2 className="font-mono font-black text-sm text-white tracking-wider uppercase">BROWN & WHITE KDS</h2>
                    <p className="text-[8px] text-white/50 font-mono tracking-widest">STATION OVERVIEW</p>
                  </div>
                </div>
              ) : (
          ...
          <nav className="flex-1 py-4 space-y-1 overflow-y-auto no-scrollbar">
            {[
              ...
              { id: "New", label: "New Orders", icon: Bell, badge: pendingOrders.length, pulse: pendingOrders.length > 0 },
              { id: "Ready", label: "Ready", icon: BadgeCheck, badge: readyOrders.length },
              { id: "Completed", label: "Completed", icon: CheckSquare, badge: completedOrders.length },
              { id: "Delayed", label: "Delayed Orders", icon: AlertTriangle, badge: delayedOrdersCount, critical: delayedOrdersCount > 0 },
              { id: "Timers", label: "Kitchen Timers", icon: Clock3, badge: timers.filter(t => t.running).length },
              { id: "Inventory", label: "Ingredient Alerts", icon: Database, badge: inventory.filter(i => i.status !== "OK").length },
              ...
              return (
                <button
                  ...
                  title={item.label}
                >
                  <IconComp className={`w-5 h-5 shrink-0 ${sidebarCollapsed ? "mx-auto" : "mr-3"}`} />
                  
                  {!sidebarCollapsed && (
                    <span className="text-xs uppercase font-mono tracking-wider">{item.label}</span>
                  )}
            ...
            {!sidebarCollapsed && (
              <div className="flex items-center space-x-3 bg-white/5 p-2.5 rounded-xl border border-[#3E2415]">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] text-white font-mono font-black uppercase truncate">{currentChef.name}</p>
                  <p className="text-[8px] text-white/50 font-mono tracking-wider truncate">{currentChef.role}</p>
                </div>
              </div>
            )}

            {/* Quick Actions (White / Dark Brown Styled) */}
            <div className={`flex ${sidebarCollapsed ? "flex-col items-center space-y-3" : "justify-around"} text-white/70`}>
              {/* Fullscreen toggle */}
              <button 
        ...
        <main className="flex-1 flex flex-col bg-[#1A0F08] overflow-y-auto p-6 space-y-6">
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-[#3E2415] pb-4 gap-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  ...
                  >
                    ...
                    <option value="Prep & Assembly">Prep & Assembly Station</option>
                    <option value="Patisserie / Desserts">Desserts Station</option>
                  </select>
                  <span className="text-white/30 text-[10px]">•</span>
                  <span className="text-[10px] font-mono text-white/60">{activeChefsCount} Chefs Clocked In</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-end space-x-4">
              <div className="text-right">
                <span className="text-white/40 font-mono text-[10px] tracking-widest uppercase block">KITCHEN SYSTEM CLOCK</span>
            ...
            <div className="space-y-6 animate-fadeIn" id="kds-dashboard-page">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                >
                  <div className="flex justify-between items-center text-white/50">
                    <span className="text-[10px] uppercase font-mono tracking-wider font-bold">New Pending</span>
                    <Bell className="w-4 h-4" />
                  </div>
                  <div className="mt-2 flex items-baseline space-x-2">
                    <span className="text-3xl font-mono font-black">{pendingOrders.length}</span>
                    <span className="text-[10px] opacity-70">tickets</span>
                  </div>
                  <div className="mt-2 text-[10px] font-mono opacity-80">Needs immediate accept</div>
                </div>
                ...
                >
                  <div className="flex justify-between items-center text-white/50">
                    <span className="text-[10px] uppercase font-mono tracking-wider font-bold">Preparing / Cooking</span>
                    <Flame className="w-4 h-4" />
                  </div>
                  <div className="mt-2 flex items-baseline space-x-2">
                    <span className="text-3xl font-mono font-black text-white">{preparingOrders.length + cookingOrders.length}</span>
                    <span className="text-[10px] text-white/60">active</span>
                  </div>
                  <div className="mt-2 text-[10px] font-mono text-white/50">Stove / Prep in progress</div>
                </div>
                ...
                >
                  <div className="flex justify-between items-center text-white/50">
                    <span className="text-[10px] uppercase font-mono tracking-wider font-bold">Ready At Pass</span>
                    <Check className="w-4 h-4" />
                  </div>
                  <div className="mt-2 flex items-baseline space-x-2">
                    <span className="text-3xl font-mono font-black text-white">{readyOrders.length}</span>
                  ...
                  <div className="mt-2 text-[10px] text-white/40 font-mono flex items-center space-x-1">
                    ...
                    <span>{readyOrders.length > 0 ? `${readyOrders.length} waiting >5m` : "Clear at pass"}</span>
                  </div>
                </div>
                ...
                >
                  <div className="flex justify-between items-center text-white/50">
                    <span className="text-[10px] uppercase font-mono tracking-wider font-bold">Kitchen Efficiency</span>
                    <TrendingUp className="w-4 h-4" />
                  </div>
                  <div className="mt-2 flex items-baseline space-x-2">
                    <span className="text-3xl font-mono font-black text-white">87%</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-[#120904] border border-[#3E2415] p-6 rounded-2xl space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="font-mono font-bold text-sm tracking-wide text-white uppercase">Hourly Order Volume</h3>
                    <span className="text-[10px] font-mono text-white/50">Last 6 Hours</span>
                  </div>
                    ...
                    {[
                      ...
                      return (
                        <div key={idx} className="flex flex-col items-center flex-1 group">
                          ...
                          <div className="w-6 bg-white/25 border border-white/40 rounded-t-md hover:bg-white transition-all duration-200" style={{ height: `${pct}%` }} />
                          <span className="text-[8px] font-mono text-white/40 mt-2">{d.hr}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="bg-[#120904] border border-[#3E2415] p-6 rounded-2xl space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="font-mono font-bold text-sm tracking-wide text-white uppercase">Category Distribution</h3>
                    <span className="text-[10px] font-mono text-white/50">Real-time counts</span>
                  </div>
                    ...
                    {[
                      ...
                      <div key={idx} className="space-y-1">
                        <div className="flex justify-between text-[10px] font-mono">
                          ...
                          <span className="text-white font-bold">{c.val} items</span>
                        </div>
                        <div className="w-full bg-[#1A0F08] h-2 rounded-full overflow-hidden border border-[#3E2415]">
                          <div className="bg-white h-full" style={{ width: `${(c.val / 80) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-7 bg-[#120904] border border-[#3E2415] p-5 rounded-2xl space-y-4">
                  <h3 className="font-mono font-bold text-sm text-white uppercase">Live Ticket Feed Ticker</h3>
                  <div className="divide-y divide-[#3E2415] max-h-48 overflow-y-auto no-scrollbar space-y-1">
                    {orders.slice(0, 5).map((o, idx) => (
                      <div key={idx} className="flex justify-between items-center py-2 text-xs">
                        <div className="flex items-center space-x-3">
                          <span className="text-white font-mono font-black">{o.id}</span>
                          <span className="text-[10px] bg-white/10 px-2 py-0.5 rounded text-white/80">{o.table}</span>
                        </div>
                        <div className="flex items-center space-x-4">
                          ...
                          <span className={`px-2 py-0.5 text-[9px] font-mono rounded font-bold uppercase border ${
                            o.status === "Ready" ? "bg-white text-[#1A0F08] border-white" : "bg-transparent text-white border-white/20"
                          }`}>
                            {o.status}
                          </span>
                        </div>
                ...
                <div className="lg:col-span-5 bg-[#120904] border border-[#3E2415] p-5 rounded-2xl space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="font-mono font-bold text-sm text-white uppercase">Ingredient Watch</h3>
                    <Database className="w-4 h-4 text-white" />
                  </div>
                  <div className="space-y-2.5 max-h-48 overflow-y-auto no-scrollbar">
                    {inventory.filter(i => i.status !== "OK").map((inv, idx) => (
                      <div key={idx} className="flex justify-between items-center bg-white/5 border border-white/20 p-2.5 rounded-xl">
                        <div>
                          <p className="text-xs font-bold text-white">{inv.item}</p>
                          <p className="text-[9px] text-white/50 font-mono">Min: {inv.min} units</p>
                        </div>
                        <div className="text-right">
                          <span className="text-xs font-mono font-black text-white block">{inv.current} left</span>
                          <button 
          ...
          {activeTab === "New" && (
            <div className="space-y-6 animate-fadeIn" id="kds-new-orders-page">
              <div className="flex justify-between items-center">
                <h2 className="font-mono font-black text-lg text-white">INCOMING TICKETS</h2>
                <span className="text-xs font-mono text-white/50">Auto-refresh: 5s • Large click targets</span>
              </div>
              ...
              {pendingOrders.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-[#120904] border border-[#3E2415] rounded-2xl text-center space-y-4">
                  ...
                  <div>
                    <h3 className="font-mono text-lg text-white font-black">ALL ORDERS CLEARED</h3>
                    <p className="text-xs text-white/50 max-w-sm mt-1">Ready for the next rush. Station on standby.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {pendingOrders.map((ord) => {
                    const orderAgeMins = Math.floor((currentTime.getTime() - new Date(ord.createdAt).getTime()) / 60000);
                    ...
                    return (
                      ...
                      >
                        {/* Header block */}
                        <div className={`p-4 ${isUrgent ? "bg-white text-[#1A0F08]" : "bg-[#1A0F08] border-b border-[#3E2415] text-white"} flex justify-between items-center`}>
                          <div>
                            <span className={`text-xs font-mono px-2 py-0.5 rounded font-black border ${isUrgent ? "bg-[#1A0F08] text-white border-transparent" : "bg-white/10 text-white border-white/20"}`}>
                              {ord.id}
                            </span>
                            <span className="text-[10px] font-bold ml-2 font-mono uppercase">
                              {ord.table === "Delivery" ? "🚚 DELIV" : ord.table.startsWith("Pickup") ? "🥡 PICK" : `🍽️ ${ord.table}`}
                            </span>
                          </div>
                        ...
                        <div className="px-4 py-2 border-b border-[#3E2415]/40 bg-[#1A0F08]/10 flex justify-between items-center text-[10px]">
                          <span className="text-white/60 font-mono">Guest: <strong className="text-white">{ord.customerName}</strong></span>
                          <span className="text-white/40 font-mono">{new Date(ord.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        </div>

                        {/* Food List (Main focus, very clear, 48px Qty target) */}
                        <div className="p-5 flex-1 overflow-y-auto no-scrollbar space-y-4">
                          {ord.items.map((item, idx) => (
                            <div key={idx} className="flex items-start justify-between border-b border-[#3E2415]/50 pb-3 last:border-0">
                          ...
                          {ord.notes && (
                            <div className="bg-white text-[#1A0F08] border border-white p-2.5 rounded-xl text-[10px] font-mono font-bold mt-2">
                              <span className="block uppercase underline mb-0.5">⚠️ PREP ALERT:</span>
                        ...
                        <div className="p-4 bg-[#1A0F08] border-t border-[#3E2415] space-y-3 shrink-0">
                          ...
                          <div className="grid grid-cols-4 gap-2">
                            ...
                            <button 
                              onClick={() => updateStatusWithAudio(ord.id, "Ready")}
          ...
          {activeTab === "Preparing" && (
            <div className="space-y-6 animate-fadeIn" id="kds-preparing-page">
              <div className="flex justify-between items-center">
                <h2 className="font-mono font-black text-lg text-white">PREPARATION CHECKLIST</h2>
                <span className="text-xs font-mono text-white/50">{preparingOrders.length} tickets in preparation</span>
              </div>
              ...
              {preparingOrders.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-[#120904] border border-[#3E2415] rounded-2xl text-center space-y-4">
                  ...
                  <div>
                    <h3 className="font-mono text-lg text-white font-black">NO ACTIVE PREPARATION</h3>
                    <p className="text-xs text-white/50 max-w-sm mt-1">Accept tickets from incoming tab to initialize checklist loops.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {preparingOrders.map((ord) => {
                    ...
                    const tasks = ["Gather ingredients", "Prep luxury patties", "Assemble bespoke sandwich", "Garnish & inspect"];
                    const orderChecklist = checklistState[ord.id] || {};
                    const checkedCount = tasks.filter(t => orderChecklist[t]).length;
                    const progressPct = (checkedCount / tasks.length) * 100;

                    return (
                      <div key={ord.id} className="bg-[#120904] border border-[#3E2415] rounded-2xl flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="p-4 bg-[#1A0F08] border-b border-[#3E2415] flex justify-between items-center">
                          <div className="flex items-center space-x-3">
                            ...
                            <span className="text-xs text-white/80 font-mono uppercase">{ord.table}</span>
                          </div>
                          <span className="text-xs font-mono text-white/50">Elapsed: {orderAgeMins}m</span>
                        </div>
                        ...
                        <div className="p-5 flex-1 space-y-4">
                          <div>
                            <div className="flex justify-between text-[10px] font-mono text-white/60 mb-1">
                              ...
                              <span className="text-white font-bold">{checkedCount} / {tasks.length}</span>
                            </div>
                            <div className="w-full bg-[#1A0F08] h-2 rounded-full overflow-hidden border border-[#3E2415]">
                              <div className="bg-white h-full transition-all duration-300" style={{ width: `${progressPct}%` }} />
                            </div>
                          </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className={`w-5 h-5 rounded border mr-3 flex items-center justify-center transition-all ${
                                    isChecked ? "bg-white border-white text-[#120904]" : "border-white/30 bg-[#1A0F08]"
                                  }`}>
                                    {isChecked && <Check className="w-4 h-4 stroke-[4]" />}
                                  </div>
                                  <span className="text-xs font-mono uppercase font-bold">{task}</span>
                                </label>
                              );
                            })}
                          </div>
                          ...
                          <div className="border-t border-[#3E2415] pt-3">
                            <span className="text-[9px] font-mono text-white/40 block uppercase">TICKET SPECIFICATION:</span>
                            {ord.items.map((i, idx) => (
                              <p key={idx} className="text-xs text-white font-mono uppercase font-bold mt-1">
                                {i.quantity}x {i.name} {i.customizations.length > 0 && `(${i.customizations.join(", ")})`}
                              </p>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-[#1A0F08] border-t border-[#3E2415] grid grid-cols-3 gap-2">
                          ...
                          <button 
                            onClick={() => updateStatusWithAudio(ord.id, "Cooking")}
          ...
          {activeTab === "Cooking" && (
            <div className="space-y-6 animate-fadeIn" id="kds-cooking-page">
              <div className="flex justify-between items-center">
                <h2 className="font-mono font-black text-lg text-white">LIVE STATION TIMERS</h2>
                <span className="text-xs font-mono text-white/50">Auto-timer decrements • Keep watch</span>
              </div>
              ...
              {orders.filter(o => o.status === "Cooking").length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-[#120904] border border-[#3E2415] rounded-2xl text-center space-y-4">
                  ...
                  <div>
                    <h3 className="font-mono text-lg text-white font-black">NO ACTIVE COOKING TIMERS</h3>
                    <p className="text-xs text-white/50 max-w-sm mt-1">Start cooking from the Preparing list. Timers automatically trigger here.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {orders.filter(o => o.status === "Cooking").map((ord) => {
                    const timerSeconds = ord.timeRemaining !== undefined ? ord.timeRemaining : 485;
                    const minutes = Math.floor(timerSeconds / 60);
                    const seconds = timerSeconds % 60;
                    
                    const ratio = timerSeconds / 600;
                    // Standard color is white, flashing white for urgent alert
                    const strokeColor = ratio < 0.25 ? "stroke-white animate-pulse" : "stroke-white/80";
                    const timerColorClass = ratio < 0.25 ? "text-white animate-pulse underline font-black" : "text-white";

                    return (
                      <div key={ord.id} className="bg-[#120904] border border-[#3E2415] rounded-2xl p-6 flex flex-col md:flex-row gap-6 items-center">
                        
                        {/* Massive Timer with Progress Ring */}
                        <div className="relative w-36 h-36 shrink-0 flex items-center justify-center">
                          <svg className="w-full h-full transform -rotate-90">
                            <circle cx="72" cy="72" r="64" className="stroke-white/10 fill-none stroke-[6]" />
                            <circle 
                              cx="72" 
                              cy="72" 
                              r="64" 
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <span className="text-[8px] font-mono text-white/50 tracking-wider">TIMER</span>
                            <span className={`text-3xl font-mono font-black ${timerColorClass}`}>
                              {String(minutes).padStart(2, '0')}:{String(seconds).padStart(2, '0')}
                            </span>
                            <span className="text-[9px] font-mono text-white/75 mt-1">{ord.id}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col justify-between h-full space-y-4 w-full">
                          <div>
                            <div className="flex justify-between items-start">
                              <div>
                                <h3 className="font-mono font-black text-sm text-white uppercase">{ord.table}</h3>
                                <p className="text-[10px] text-white/50 font-mono mt-0.5">Station: {selectedStation}</p>
                              </div>
                            ...
                            <div className="mt-3 space-y-1 bg-[#1A0F08] p-2.5 rounded-xl border border-[#3E2415] max-h-24 overflow-y-auto no-scrollbar">
                              {ord.items.map((i, idx) => (
                                <div key={idx} className="flex items-center text-xs text-white/90">
                                  ...
                                  <span className="font-mono uppercase">{i.quantity}x {i.name}</span>
                                </div>
                              ))}
                            </div>
                          </div>
          ...
          {activeTab === "Ready" && (
            <div className="space-y-6 animate-fadeIn" id="kds-ready-page">
              <div className="flex justify-between items-center">
                <h2 className="font-mono font-black text-lg text-white">READY FOR PICKUP</h2>
                <span className="text-xs font-mono text-white/50">{readyOrders.length} tickets ready at the pass</span>
              </div>
              ...
              {readyOrders.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-[#120904] border border-[#3E2415] rounded-2xl text-center space-y-4">
                  ...
                  <div>
                    <h3 className="font-mono text-lg text-white font-black">PASS IS CLEAR</h3>
                    <p className="text-xs text-white/50 max-w-sm mt-1">Stove plated items register here to synchronize floor waiters.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {readyOrders.map((ord) => {
                    ...
                    return (
                      <div key={ord.id} className="bg-[#120904] border-2 border-white rounded-2xl p-5 flex flex-col justify-between min-h-[320px]">
                        ...
                        <div>
                          ...
                          <div className="mt-4 space-y-2 border-t border-[#3E2415] pt-3">
                            <span className="text-[9px] font-mono text-white/40 block uppercase">Pack Checklist:</span>
                            {ord.items.map((i, idx) => (
                              <div key={idx} className="flex justify-between text-xs text-white font-mono uppercase font-bold">
                                <span>{i.name}</span>
                        ...
                        <div className="mt-6 space-y-2">
                          <div className="grid grid-cols-2 gap-2">
                            <button 
                              onClick={() => {
                                playSound("success");
                                alert(`Receipt Label printed for Ticket ${ord.id}`);
                              }}
                            ...
                            <button 
                              onClick={() => {
                                playSound("beep");
                                alert(`Floor waiters paged for Ticket ${ord.id}`);
                              }}
                          ...
                          <button 
                            onClick={() => updateStatusWithAudio(ord.id, "Completed")}
          ...
          {/* ---------------------------------------------------------------------------------- */}
          {activeTab === "Completed" && (
            <div className="bg-[#120904] border border-[#3E2415] rounded-2xl p-6 space-y-6 animate-fadeIn" id="kds-completed-page">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="font-mono font-black text-lg text-white">CHRONOLOGICAL DISPATCH HISTORY</h2>
                  <p className="text-xs text-white/50 mt-1">Completed items archive database</p>
                </div>
                    ...
                    <tbody className="divide-y divide-[#3E2415] font-mono text-white/80">
                      {completedOrders.map((o) => (
                        <tr key={o.id} className="hover:bg-white/5">
                          ...
                          <td className="p-4 uppercase">{o.customerName} ({o.table})</td>
                          <td className="p-4">{selectedStation}</td>
                          <td className="p-4">{new Date(o.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</td>
          ...
          {/* ---------------------------------------------------------------------------------- */}
          {activeTab === "Delayed" && (
            <div className="bg-white/5 border-2 border-white/20 rounded-2xl p-6 space-y-6 animate-fadeIn" id="kds-delayed-page">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="font-mono font-black text-lg text-white">CRITICAL DELAY ALERT PANEL</h2>
                  <p className="text-xs text-white/50 mt-1">Requires supervisor reallocations</p>
                </div>
              ...
              {delayedOrdersCount === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <div>
                    <h3 className="font-mono text-lg text-white font-black">STATION RUNNING AT 100% TIMING</h3>
                    <p className="text-xs text-white/50 max-w-sm mt-1">All tickets are currently preparing within healthy timing bounds.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {orders.filter(o => {
                    ...
                    return (
                      <div key={ord.id} className="bg-[#120904] border border-white rounded-2xl p-5 flex flex-col justify-between">
                        
                        <div>
                          <div className="flex justify-between">
                            <span className="text-lg font-mono font-black text-white">{ord.id}</span>
                            <span className="text-xs font-mono text-white bg-white/10 px-2 py-0.5 rounded font-black">Elapsed: {orderAgeMins} Mins</span>
                          </div>
                          
                          <p className="text-xs text-white/70 font-mono mt-1 uppercase">{ord.table} • Guest: {ord.customerName}</p>

                          {/* Food Summary */}
                          <div className="mt-3 bg-[#1A0F08] border border-[#3E2415] p-2.5 rounded-xl space-y-1">
                            {ord.items.map((i, idx) => (
                              <p key={idx} className="text-xs text-white font-mono font-bold uppercase">
                                {i.quantity}x {i.name}
                              </p>
                            ))}
                          </div>
                        ...
                        <div className="mt-6 space-y-3">
                          <div className="flex justify-between items-center text-[10px] font-mono text-white/50">
                            ...
                            <span className="text-white font-black uppercase">
                              {ord.notes?.includes("DELAYED") ? ord.notes.split("]")[0].replace("DELAYED: [", "") : "Requires Filing"}
                            </span>
                          </div>
          ...
          {/* ---------------------------------------------------------------------------------- */}
          {activeTab === "Timers" && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fadeIn" id="kds-timers-page">
              ...
              <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
                {timers.map((t) => {
                  ...
                  const strokeColor = ratio < 0.2 ? "bg-white animate-pulse" : "bg-white/70";

                  return (
                    <div key={t.id} className="bg-[#120904] border border-[#3E2415] p-6 rounded-2xl flex flex-col justify-between space-y-4">
                      
                      <div className="flex justify-between items-center">
                        <div>
                          <h4 className="font-mono font-black text-xs text-white uppercase">{t.label}</h4>
                          <span className="text-[8px] font-mono text-white/40 block mt-0.5 uppercase">Type: {t.type}</span>
                        </div>
                        <span className={`w-2.5 h-2.5 rounded-full ${t.running ? "bg-white animate-ping" : "bg-white/20"}`} />
                      </div>
                      ...
                      <div className="w-full bg-[#1A0F08] h-1.5 rounded-full overflow-hidden border border-[#3E2415]">
                        <div className={`${strokeColor} h-full transition-all`} style={{ width: `${ratio * 100}%` }} />
                      </div>

                      <div className="grid grid-cols-3 gap-2 shrink-0">
                        <button 
                          onClick={() => {
                            setTimers(timers.map(ti => ti.id === t.id ? { ...ti, running: !ti.running } : ti));
                            playSound("beep");
                          }}
                        ...
                        <button 
                          onClick={() => {
                            setTimers(timers.map(ti => ti.id === t.id ? { ...ti, time: t.maxTime, running: false } : ti));
                            playSound("beep");
                          }}
                          className="bg-transparent hover:bg-white/10 border border-white/20 text-[10px] font-mono text-white py-1.5 rounded-lg"
                        >
              ...
              <div className="lg:col-span-4 bg-[#120904] border border-[#3E2415] p-6 rounded-2xl space-y-4">
                <h4 className="font-mono font-black text-sm text-white uppercase">Launch Timer</h4>
                <div className="space-y-4">
                  
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-white/50 uppercase block">Label Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-white/50 uppercase block">Station Type</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-white/50 uppercase block">Duration (Secs)</label>
                    <input 
                      type="number" 
                      placeholder="e.g. 180"
          ...
          {/* ---------------------------------------------------------------------------------- */}
          {activeTab === "Inventory" && (
            <div className="bg-[#120904] border border-[#3E2415] rounded-2xl p-6 space-y-6 animate-fadeIn" id="kds-inventory-alerts-page">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="font-mono font-black text-lg text-white uppercase">Ingredient Threshold Alerts</h2>
                  <p className="text-xs text-white/50 mt-1">Real-time stock monitoring loops to secure preparation</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {inventory.map((inv, idx) => {
                  const isCritical = inv.status === "Critical" || inv.current === 0;
                  ...
                  return (
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-start">
                          <h4 className="text-sm font-mono font-black uppercase text-white">{inv.item}</h4>
                        ...
                        <p className="text-[10px] text-white/50 font-mono mt-1">Min Threshold: {inv.min} units</p>
                      </div>

                      <div className="flex justify-between items-end border-t border-white/10 pt-3">
                        <div>
                          <span className="text-[8px] font-mono text-white/40 block uppercase">CURRENT LEVEL</span>
                          <span className="font-mono text-base font-black text-white">{inv.current} left</span>
                        </div>
                        <button 
                          onClick={() => handleIngredientRestock(inv.item)}
          ...
          {/* ---------------------------------------------------------------------------------- */}
          {activeTab === "Reports" && (
            <div className="bg-[#120904] border border-[#3E2415] rounded-2xl p-6 space-y-6 animate-fadeIn" id="kds-reports-page">
              <div className="flex justify-between items-center border-b border-[#3E2415] pb-4">
                <div>
                  <h2 className="font-mono font-black text-lg text-white">SHIFT SPEED REPORTS</h2>
                  <p className="text-xs text-white/50 mt-1">Review speed, efficiency, and cancellation metrics</p>
                </div>
                <button 
                  onClick={() => alert("Downloading shifting reports summary...")}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-[#1A0F08] p-5 rounded-2xl border border-[#3E2415] space-y-3">
                  <h4 className="text-white/50 font-mono text-xs uppercase">Overall Dispatch Speed</h4>
                  <p className="text-2xl font-mono font-black text-white">11.8 min</p>
                  <p className="text-[10px] text-white/60 font-sans">Avg prep time. Down 1.2 min from yesterday.</p>
                </div>

                <div className="bg-[#1A0F08] p-5 rounded-2xl border border-[#3E2415] space-y-3">
                  <h4 className="text-white/50 font-mono text-xs uppercase">Cancellation / Waste Rate</h4>
                  <p className="text-2xl font-mono font-black text-white">1.4%</p>
                  <p className="text-[10px] text-white/60 font-sans">Rejected orders or custom remakes.</p>
                </div>

                <div className="bg-[#1A0F08] p-5 rounded-2xl border border-[#3E2415] space-y-3">
                  <h4 className="text-white/50 font-mono text-xs uppercase">Checklist Compliance Score</h4>
                  <p className="text-2xl font-mono font-black text-white">98.2%</p>
                  <p className="text-[10px] text-white/60 font-sans">Checklist completion on luxury tickets.</p>
                </div>
              ...
              <div className="space-y-4 pt-4">
                <h3 className="font-mono font-bold text-sm text-white uppercase">Station Efficiency Table</h3>
                <div className="divide-y divide-[#3E2415] font-mono text-xs">
                  {[
                    { station: "Grill Station #2", completed: 48, speed: "11.2 min", efficiency: "94%" },
                    { station: "Fryer Station #1", completed: 32, speed: "4.5 min", efficiency: "98%" },
                    { station: "Prep & Assembly", completed: 51, speed: "6.8 min", efficiency: "91%" },
                    { station: "Patisserie / Desserts", completed: 14, speed: "14.1 min", efficiency: "87%" }
                  ].map((s, idx) => (
                    <div key={idx} className="flex justify-between items-center py-3">
                      <span className="font-bold text-white uppercase">{s.station}</span>
                      <div className="flex space-x-6 text-white/60">
                        <span>{s.completed} Tickets</span>
                        <span>Speed: {s.speed}</span>
                        <span className="text-white font-black">Score: {s.efficiency}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="bg-[#120904] border-2 border-white rounded-3xl shadow-2xl w-[320px] h-[440px] flex flex-col justify-between overflow-hidden">
            ...
            <div className="bg-white p-4 flex justify-between items-center text-[#120904]">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-mono font-black text-xs uppercase tracking-wider">AI Coordinator</span>
              </div>
              <button 
            ...
            <div className="px-3 py-1 bg-[#120904] flex gap-1 overflow-x-auto no-scrollbar border-t border-[#3E2415]">
              {[
                { label: "Prep times?", q: "What is my current queue preparation time analysis?" },
                { label: "Patties low?", q: "Beef patties are critical, how do I handle menu block?" },
                { label: "Optimize?", q: "How should I optimize chef station roles for burgers?" }
              ].map((p, idx) => (
                <button
                  key={idx}
                  onClick={() => {
          ...
          <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            ...
            >
              <div className="text-center">
                ...
                <h3 className="font-mono font-black text-base text-white uppercase tracking-wide">Manager Pin Required</h3>
                <p className="text-[10px] text-white/50 font-mono mt-1">Action requires manager authorization bypass</p>
              </div>
              ...
              <div className="text-center">
                <span className="text-[9px] text-white/40 font-mono uppercase">Shift Code: 1234, 7777, 0000</span>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            ...
            >
              <div className="text-center">
                ...
                <h3 className="font-mono font-black text-base text-white uppercase tracking-wide">File Delay Reason</h3>
                <p className="text-[10px] text-white/50 font-mono mt-1">Audit log updates waiter and reception dashboards</p>
              </div>

              <div className="space-y-4">
                
                <div className="space-y-1">
                  <label className="text-[9px] font-mono text-white/50 uppercase block">Primary Reason</label>
                  <select 
                    value={delayReason}
                    onChange={(e) => setDelayReason(e.target.value)}
                    className="w-full bg-[#1A0F08] border border-[#3E2415] text-xs text-white p-3 rounded-xl focus:outline-none focus:border-white"
                  >
                    <option value="Kitchen backup">Kitchen backup / High Traffic</option>
                    <option value="Equipment failure">Equipment failure / Grill down</option>
                    <option value="Ingredient shortage">Ingredient shortage</option>
                    <option value="Complex order">Complex burger specifications</option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] font-mono text-white/50 uppercase block">Extra Notes</label>
                  <textarea 
          ...
          <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            ...
            >
              <div className="text-center">
                ...
                <h3 className="font-mono font-black text-base text-white uppercase tracking-wide">Restock Refill</h3>
                <p className="text-[10px] text-white/50 font-mono mt-1">Replenish item: <strong className="text-white">{selectedIngredient}</strong></p>
              </div>

              <div className="space-y-4">
                
                <div className="space-y-1">
                  <label className="text-[9px] font-mono text-white/50 uppercase block">Refill Qty (units)</label>
                  <input 
                    type="number"
                    value={restockQty}
