...
export default function WaiterDashboard({
  ...
  const [isAuthenticated, setIsAuthenticated] =
    React.useState<boolean>(!!authenticatedStaff);
  const [employeeId, setEmployeeId] = React.useState<string>(
    authenticatedStaff?.id || "",
  );
  const [pin, setPin] = React.useState<string>("");
  const [branch, setBranch] = React.useState<string>("Manhattan Central");
  const [waiterName, setWaiterName] = React.useState<string>(
    authenticatedStaff?.name || "Emily Waiter",
  );
  ...
  const [tableOverrides, setTableOverrides] = React.useState<
    Record<string, "Reserved" | "Cleaning" | "Broken" | "Available">
  >({});

  // Active Bottom Navigation Tab
  const [activeTab, setActiveTab] = React.useState<
    "Tables" | "New" | "MyOrders" | "Settle" | "Profile"
  >("Tables");
  ...
  const [recentlyPlacedOrderId, setRecentlyPlacedOrderId] =
  ...
  const [tableFilter, setTableFilter] = React.useState<
    "All" | "Available" | "Occupied" | "Reserved" | "MyTables"
  >("All");
  const [longPressedTable, setLongPressedTable] = React.useState<string | null>(
    null,
  ); // To show table management context menu
  ...
  const [isReceiptPrinting, setIsReceiptPrinting] =
    React.useState<boolean>(false);
  const [receiptPrintedMessage, setReceiptPrintedMessage] =
    React.useState<string>("");

  // Split bill states
  const [isSplitBill, setIsSplitBill] = React.useState<boolean>(false);
  const [splitType, setSplitType] = React.useState<"Amount" | "Items">(
    "Amount",
  );
  const [splitSharesCount, setSplitSharesCount] = React.useState<number>(2);
  const [paidShares, setPaidShares] = React.useState<number[]>([]);
  const [splitItemSelections, setSplitItemSelections] = React.useState<
  ...
  const [managerMessageSent, setManagerMessageSent] =
    React.useState<boolean>(false);
  const [activeLanguage, setActiveLanguage] = React.useState<string>("English");
  const [notificationsEnabled, setNotificationsEnabled] =
    React.useState<boolean>(true);
  const [myOrdersFilter, setMyOrdersFilter] = React.useState<"My" | "All">(
    "My",
  );
  ...
  const handleClockIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const foundEmp = employees.find(
        (emp: any) => emp.id.toUpperCase() === lookupId,
      );
      ...
      if (foundEmp.isAccessEnabled === false) {
        alert(
          "❌ ACCESS DENIED: Your account is disabled. Contact manager or owner.",
        );
        return;
      }
    ...
    if (prev.length > 0) {
      ...
      orders.forEach((currentOrder) => {
        ...
        if (
          ...
          (!oldOrder || oldOrder.status !== "Ready")
        ) {
  ...
  const toggleCustomization = (option: string) => {
    if (selectedCustomizations.includes(option)) {
      setSelectedCustomizations(
        selectedCustomizations.filter((o) => o !== option),
      );
    } else {
  ...
  const getSettleTotals = (activeOrder: Order | undefined) => {
    ...
    if (isSplitBill && splitType === "Items") {
      ...
      const fullSubtotal = activeOrder.items.reduce(
        (sum, item) => sum + item.price * item.quantity,
        0,
      );
      subtotal = fullSubtotal / splitSharesCount;
    } else {
      subtotal = activeOrder.items.reduce(
        (sum, item) => sum + item.price * item.quantity,
        0,
      );
    }
  ...
  const handleProcessPayment = () => {
    ...
    if (
      activeOrderForSettle.type === "Dine-in" &&
      activeOrderForSettle.status !== "Ready" &&
      activeOrderForSettle.status !== "Completed"
    ) {
      alert(
        "⚠️ Kitchen Prep Not Ready: This order is still being prepared by the kitchen. The bill receipt cannot be generated and payment cannot be processed until the chef marks the food as READY.",
      );
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (isSplitBill) {
        if (splitType === "Amount") {
          ...
          const nextShareIdx = Array.from({
            length: splitSharesCount,
          }).findIndex((_, idx) => !paidShares.includes(idx));
          const newPaidShares = [...paidShares, nextShareIdx].filter(
            (idx) => idx !== -1,
          );
          ...
          const remainingItems = activeOrderForSettle.items
            .map((item) => {
              const selectedQty = splitItemSelections[item.id] || 0;
              return {
                ...item,
          ...
          if (remainingItems.length > 0) {
            ...
            onUpdateOrderStatus(
              activeOrderForSettle.id,
              activeOrderForSettle.status,
              {
  ...
  return (
    <div
      className="w-full bg-slate-950 min-h-screen flex flex-col justify-between overflow-hidden relative font-sans text-slate-100"
      id="waiter-pocket-pos"
    >
      ...
      <div className="bg-slate-900 px-4 py-3 border-b border-slate-800 flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="font-serif font-black text-xs tracking-wide text-[#D4AF37]">
              AURA FLOOR
            </h1>
      ...
      {notification && (
        ...
        >
          ...
          <div className="flex-grow min-w-0">
            <span className="text-[8px] font-mono text-amber-200 uppercase block tracking-widest font-black">
              Kitchen Order Notification
            </span>
            <span className="text-xs font-serif font-bold block mt-0.5 truncate text-white">
              {notification.message}
            </span>
            <span className="text-[10px] font-mono text-amber-100 block mt-0.5">
              {notification.sub}
            </span>
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar bg-slate-900 pb-16">
        ...
        {!isAuthenticated ? (
          <div
            className="p-5 flex flex-col justify-center min-h-[70vh] space-y-6 animate-fadeIn"
            id="waiter-login-screen"
          >
            <div className="text-center space-y-2">
              ...
              <h2 className="text-lg font-serif font-bold text-white tracking-wide mt-2">
                Staff Terminal Authentication
              </h2>
              <p className="text-xs text-slate-400 px-6">
                Input your Employee ID and 4-digit security PIN to clock-in for
                your shift.
              </p>
            </div>
            ...
            <form onSubmit={handleClockIn} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-mono uppercase tracking-widest text-[#D4AF37] block">
                  Employee ID
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-mono uppercase tracking-widest text-[#D4AF37] block text-center">
                  Security PIN
                </label>
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const employees = await res.json();
                          const foundEmp = employees.find(
                            (emp: any) => emp.id.toUpperCase() === staff.id,
                          );
                          if (foundEmp) {
                            const attendanceRes = await fetch(
                              "/api/employees/attendance/auto",
                              {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center text-[8px] font-mono text-slate-500 mt-0.5">
                        <span>{staff.id}</span>
                        <span className="text-[#D4AF37]/80 font-bold">
                          PIN: {staff.pin}
                        </span>
                      </div>
                    </button>
              ...
              <div className="space-y-4 animate-fadeIn" id="tables-floor-map">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {STATIC_TABLES.filter((tbl) => {
                    ...
                    if (tableFilter === "Available" && status !== "Available")
                      return false;
                    if (tableFilter === "Occupied" && status !== "Occupied")
                      return false;
                    if (tableFilter === "Reserved" && status !== "Reserved")
                      return false;
                    ...
                    let statusBg =
                      "from-slate-950 to-emerald-950/20 border-emerald-500/20";
                    let textColor = "text-emerald-400";

                    if (status === "Occupied") {
                      statusColor = "bg-rose-500";
                      statusBg =
                        "from-slate-950 to-rose-950/20 border-rose-500/20";
                      textColor = "text-rose-400";
                    } else if (status === "Reserved") {
                      statusColor = "bg-amber-500";
                      statusBg =
                        "from-slate-950 to-amber-950/20 border-amber-500/20";
                      textColor = "text-amber-400";
                    } else if (status === "Cleaning") {
                      statusColor = "bg-sky-500";
                      statusBg =
                        "from-slate-950 to-sky-950/20 border-sky-500/20";
                      textColor = "text-sky-400";
                    } else if (status === "Broken") {
                      statusColor = "bg-purple-500";
                      statusBg =
                        "from-slate-950 to-purple-950/20 border-purple-500/20";
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex justify-between items-start">
                          <span
                            className={`text-[10px] font-mono font-bold uppercase tracking-wide ${textColor}`}
                          >
                            {status}
                          </span>
                          <span
                            className={`w-2.5 h-2.5 rounded-full ${statusColor} ${status === "Occupied" ? "animate-pulse" : ""}`}
                          />
                        </div>
                  ...
                  <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs flex items-end justify-center z-50">
                    <div className="bg-slate-900 border-t-2 border-[#D4AF37] rounded-t-[2.5rem] w-full max-w-md p-6 space-y-5 animate-slideUp text-slate-100">
                      ...
                      <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                        <div>
                          <h3 className="font-serif font-black text-lg text-[#D4AF37]">
                            {longPressedTable} Management
                          </h3>
                      ...
                      {getTableStatus(longPressedTable) === "Occupied" ? (
                        <div className="space-y-4">
                          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2.5">
                            <h4 className="text-[10px] font-mono text-slate-500 uppercase">
                              Active Order Parameters
                            </h4>
                            {(() => {
                              const order =
                                getTableActiveOrder(longPressedTable);
                              ...
                              return (
                                <>
                                  ...
                                  <div className="text-[10px] text-slate-400 font-sans border-t border-slate-900 pt-2">
                                    <span className="font-mono text-[8px] uppercase text-slate-500 block">
                                      Seat items
                                    </span>
                          ...
                          <div className="grid grid-cols-2 gap-2.5">
                            ...
                            <button
                              onClick={() => {
                                alert(
                                  `Bluetooth ticket print job sent to Salon-Floor-Printer-1 for ${longPressedTable}!`,
                                );
                                setLongPressedTable(null);
                              }}
                            ...
                            <button
                              onClick={() => {
                                if (
                                  confirm(
                                    `Force close ${longPressedTable} without settling? (This clears order context)`,
                                  )
                                ) {
                                  const order =
                                    getTableActiveOrder(longPressedTable);
                        ...
                        <div className="space-y-4">
                          <p className="text-xs text-slate-400">
                            Apply visual floor tags or reservation status
                            coordinates to {longPressedTable}.
                          </p>
                ...
                {wizardStep === 1 && (
                  <div className="space-y-4 animate-fadeIn">
                    <p className="text-xs text-slate-400 font-mono uppercase tracking-wider text-center">
                      Step 1: Choose Table Coordinates
                    </p>
                ...
                {wizardStep === 2 && (
                  <div className="space-y-6 py-6 text-center animate-fadeIn">
                    ...
                    <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">
                      Estimated seating capacity of table is respected
                    </p>
                    ...
                    <div className="space-y-2.5 max-h-[38vh] overflow-y-auto no-scrollbar pr-1">
                      {activeProducts
                        .filter((p) => {
                          ...
                          return (
                            matchesCategory && matchesSearch && matchesAvailable
                          );
                        })
                        .map((prod) => {
                          return (
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-sm p-5 space-y-4 text-slate-100 animate-fadeIn">
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                                ${customizingProduct.price.toFixed(2)} each
                              </p>
                            </div>
                            <button
                          ...
                          <div className="space-y-1.5">
                            <span className="text-[9px] font-mono text-slate-500 uppercase">
                              Preparation / Ingredients Extras
                            </span>
                              ...
                              <button
                                onClick={() =>
                                  setCustomizationQuantity(
                                    Math.max(1, customizationQuantity - 1),
                                  )
                                }
                          ...
                          >
                            Add {customizationQuantity} item
                            {customizationQuantity > 1 ? "s" : ""} to Cart
                          </button>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3.5">
                      ...
                      <div className="space-y-2 max-h-[22vh] overflow-y-auto no-scrollbar pr-1">
                        {cart.map((item) => (
                          <div
                            key={item.id}
                            className="flex justify-between items-start text-xs border-b border-slate-900/60 pb-2"
                          >
                ...
                {wizardStep === 5 && (
                  <div className="space-y-6 text-center py-6 animate-fadeIn">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-400 px-6">
                        Your table ticket was successfully printed at the
              ...
              <div className="space-y-4 animate-fadeIn" id="my-assigned-orders">
                    ...
                    <span className="text-[10px] font-mono text-slate-400 bg-slate-950 px-2 py-0.5 rounded border border-slate-800">
                      {
                        orders.filter((o) => {
                          const isDineInActive =
                            o.status !== "Completed" && o.type === "Dine-in";
                          ...
                          if (myOrdersFilter === "My") {
                            ...
                            return (
                              oWaiter === wName ||
                              oWaiter.includes(wName) ||
                              wName.includes(oWaiter)
                            );
                          }
                          return true;
                        }).length
                ...
                {orders.filter((o) => {
                  ...
                  <div className="text-center py-10 bg-slate-950/40 rounded-2xl border border-slate-850 space-y-3">
                    ...
                    <div>
                      <p className="text-xs font-serif font-bold text-slate-400">
                        No active tables assigned
                      </p>
                      <p className="text-[10px] text-slate-500 font-mono">
                        Seat guest tables or dispatch tickets from floor map.
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {orders
                      ...
                      .map((ord) => {
                        ...
                        const elapsedMs =
                          Date.now() - new Date(ord.createdAt).getTime();
                        ...
                        return (
                          <div
                            key={ord.id}
                            className="bg-slate-950 border border-slate-850 p-4 rounded-2xl space-y-3 shadow-lg"
                          >
                            {/* Card Header */}
                            <div className="flex justify-between items-start border-b border-slate-900 pb-2.5">
                              <div>
                                <h3 className="font-serif font-black text-sm text-white flex items-center">
                                  <span className="text-[#D4AF37] mr-1.5">
                                    {ord.table}
                                  </span>
                                ...
                                <p className="text-[8px] font-mono text-slate-500 uppercase mt-0.5">
                                  ID: {ord.id} • Seated {elapsedMinutes}m ago •
                                  Waiter: {ord.waiter || "None"}
                                </p>
                              </div>

                              {/* Status label color */}
                              <span
                                className={`px-2 py-0.5 rounded-full text-[8px] font-mono uppercase tracking-wide ${
                                  ord.status === "Ready"
                                ...
                                {ord.items.map((it, idx) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <span
                                        className={`text-[8px] uppercase tracking-wider ${itemClass}`}
                                      >
                                        {itemStatus}
                                      </span>
                                    </div>
                            ...
                            <div className="flex space-x-2 pt-1">
                              {ord.status === "Ready" ? (
                                <button
                                  onClick={() => {
                                    ...
                                    alert(
                                      `Order marked as served for ${ord.table}!`,
                                    );
                                  }}
                              ...
                              <button
                                onClick={() => {
                                  if (
                                    confirm(`Clear and release ${ord.table}?`)
                                  ) {
                                    onUpdateOrderStatus(ord.id, "Completed", {
                                      paymentStatus: "Paid",
                                    });
            ...
            {activeTab === "Settle" && (
              <div className="space-y-4 animate-fadeIn" id="payments-page">
                ...
                {!settlingTable ? (
                  <div className="space-y-3">
                    <p className="text-xs text-slate-400 font-mono uppercase tracking-wider text-center">
                      Select active table to process payment
                    </p>

                    {orders.filter(
                      (o) => o.status !== "Completed" && o.type === "Dine-in",
                    ).length === 0 ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-slate-950 p-4 rounded-2xl border border-slate-850 space-y-3.5">
                      <div className="flex justify-between items-center pb-2.5 border-b border-slate-900 text-xs">
                        <div>
                          <span className="font-serif font-black text-white text-sm block">
                            {settlingTable}
                          </span>
                          <span className="text-[8px] font-mono text-slate-500 uppercase">
                            Ticket ID: {activeOrderForSettle?.id}
                          </span>
                        </div>
                        <div className="text-right">
                          ...
                          <span className="font-mono text-[#D4AF37] font-bold">
                            {activeOrderForSettle?.items.reduce(
                              (sum, i) => sum + i.quantity,
                              0,
                            )}
                          </span>
                      ...
                      {activeOrderForSettle &&
                        ...
                        activeOrderForSettle.status !== "Completed" && (
                          <div className="bg-red-500/15 border border-red-500/30 p-3.5 rounded-xl flex items-start space-x-2 text-red-400 text-[10px] font-mono animate-pulse">
                            ...
                            <div>
                              <p className="font-bold uppercase tracking-wider text-red-500">
                                ⚠️ KDS Prep Not Ready
                              </p>
                              <p className="mt-1 text-[9px] text-slate-400 normal-case leading-normal">
                                The kitchen display system (KDS) has not marked
                                this order as "READY" yet. Receipt print &
                                payment settlement are strictly disabled until
                          ...
                          <div className="space-y-2 animate-fadeIn border-t border-slate-800/80 pt-2 text-[9px] font-mono">
                            ...
                            {splitType === "Amount" ? (
                              <div className="flex justify-between items-center bg-slate-950 p-2 rounded">
                                ...
                                <div className="flex items-center space-x-2">
                                  <button
                                    onClick={() =>
                                      setSplitSharesCount(
                                        Math.max(2, splitSharesCount - 1),
                                      )
                                    }
                                    className="p-0.5 bg-slate-900 text-slate-400 font-bold text-xs rounded"
                                  >
                                    -
                                  </button>
                                  <span className="font-bold text-white text-xs">
                                    {splitSharesCount} Shares
                                  </span>
                                  <button
                                    onClick={() =>
                                      setSplitSharesCount(
                                        Math.min(8, splitSharesCount + 1),
                                      )
                                    }
                                    className="p-0.5 bg-slate-900 text-slate-400 font-bold text-xs rounded"
                                  >
                              ...
                              <div className="bg-slate-950 p-2 rounded-xl space-y-2">
                                <span className="text-[8px] uppercase text-[#D4AF37] font-bold block">
                                  Select Quantities to Pay Now:
                                </span>
                                <div className="space-y-1.5 max-h-32 overflow-y-auto no-scrollbar">
                                  {activeOrderForSettle?.items.map((item) => {
                                    const selectedQty =
                                      splitItemSelections[item.id] || 0;
                                    return (
                                      <div
                                        key={item.id}
                                        className="flex justify-between items-center text-[10px] text-slate-300 py-1 border-b border-slate-900 last:border-0"
                                      >
                                        <div className="min-w-0 flex-1 pr-2">
                                          <p className="truncate text-slate-200 font-bold">
                                            {item.name}
                                          </p>
                              ...
                              <div className="bg-slate-950 p-2 rounded space-y-1">
                                <div className="flex justify-between text-[10px] text-white">
                                  ...
                                  <span className="font-bold text-[#D4AF37]">
                                    $
                                    {(totals.total / splitSharesCount).toFixed(
                                      2,
                                    )}
                                  </span>
                                </div>
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {Array.from({ length: splitSharesCount }).map(
                                    (_, idx) => {
                    ...
                    <div className="bg-slate-900 border-2 border-[#D4AF37] rounded-3xl w-full max-w-sm p-6 text-center space-y-5 animate-scaleUp">
                      <div className="space-y-1">
                        ...
                        <p className="text-[8px] font-mono text-slate-500">
                          Connected to Mobile Epson-M30 • 203 DPI
                        </p>
                      </div>
                      ...
                      <div className="bg-slate-950 p-4 border border-dashed border-slate-700 max-h-40 overflow-y-auto no-scrollbar rounded-xl text-[8px] font-mono text-left text-slate-300 uppercase space-y-1 leading-normal mx-4 select-none relative animate-pulse shadow-inner">
                        <p className="text-center font-bold tracking-widest">
                          AURA GOURMET BURGERS
                        </p>
                        <p className="text-center text-[7px] text-slate-500">
                          --- DUPLICATE RECEIPT ---
                        </p>
                        ...
                        <p className="border-b border-dashed border-slate-800 my-1 pb-1">
                          {receiptPrintedMessage}
                        </p>
                        <p className="text-center text-[#D4AF37] font-bold text-[9px] mt-2">
              ...
              >
                ...
                <div className="bg-slate-950 p-5 rounded-3xl border border-slate-850 flex items-center space-x-4 shadow-lg">
                  ...
                  <div className="min-w-0 flex-grow">
                    <h3 className="font-serif font-black text-base text-white truncate">
                      {waiterName}
                    </h3>
                    <p className="text-[9px] font-mono text-[#D4AF37] uppercase tracking-wider mt-0.5">
                      Senior Floor Server
                    </p>
                    <p className="text-[8px] font-mono text-slate-500 mt-1 uppercase">
                      ID: EMP-105 • Active Shift
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-850 space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      onClick={() => {
                        ...
                        alert(
                          isOnBreak
                            ? "You are now Clocked in and active on Salon Floor!"
                            : "Break request logged with manager.",
                        );
                      }}
                  ...
                  <div className="border-t border-slate-900/60 pt-3 space-y-2">
                      ...
                      <button
                        onClick={() => {
                          if (managerMessage.trim()) {
                            ...
                            setManagerMessage("");
                            alert(
                              "Dispatched helper coordinate message to executive KDS display & manager app.",
                            );
                          }
                        }}
                    ...
                    {managerMessageSent && (
                      <span className="text-[8px] font-mono text-emerald-400 block animate-pulse">
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {[
                    {
                      label: "Tables Closed",
                      val: stats.tablesServed,
                      icon: TableProperties,
                    },
                    {
                      label: "Tips Collected",
                      val: `$${stats.tipsEarned.toFixed(2)}`,
                      icon: Award,
                    },
                    {
                      label: "Avg Service Speed",
                      val: "94% Score",
                      icon: Activity,
                    },
                  ].map((s, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="bg-slate-950 border border-slate-850 p-3 rounded-2xl shadow text-center space-y-1.5"
                      >
                        ...
                        <div>
                          ...
                          <p className="text-[8px] text-slate-500 uppercase mt-0.5 leading-snug">
                            {s.label}
                          </p>
                        </div>
                      </div>
                ...
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-850 space-y-3.5 text-xs font-mono">
                  <span className="text-[8px] font-mono text-slate-500 uppercase block tracking-wider border-b border-slate-900 pb-1.5">
                    Terminal Parameters
                  </span>
                ...
                <button
                  onClick={() => {
                    if (
                      confirm(
                        "Proceed to clock out and terminate current terminal session?",
                      )
                    ) {
                      setIsAuthenticated(false);
                      setPin("");
      ...
      {isAuthenticated && (
        <div
          className="fixed z-100 bottom-0 left-0 right-0 h-16 bg-slate-950 border-t border-slate-900/80 flex items-center justify-around px-2 rounded-none select-none pb-0 shrink-0"
          id="waiter-sticky-bottom-navigation"
        >
          {[
            ...
            return (
              ...
              >
                <div
                  className={`p-1 rounded-xl transition-colors ${isActive ? "bg-[#D4AF37]/10" : ""}`}
                >
